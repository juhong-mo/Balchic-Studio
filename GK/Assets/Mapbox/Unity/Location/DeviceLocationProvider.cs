        ...
        IEnumerator PollLocationRoutine()
        {
			...
			while (!UnityEditor.EditorApplication.isRemoteConnected)
			{
				...
				if (null != LocationProviderFactory.Instance && null != LocationProviderFactory.Instance.DefaultLocationProvider)
				{
				 ...
				 UnityEngine.Debug.LogWarning("Remote device not connected via 'Unity Remote'. Waiting ..." + Environment.NewLine + "If Unity seems to be stuck here make sure 'Unity Remote' is running and restart Unity with your device already connected.");
            ...
            if (!_locationService.isEnabledByUser)
            {
                UnityEngine.Debug.LogError("DeviceLocationProvider: Location is not enabled by user!");
            ...
            if (maxWait < 1)
            {
                UnityEngine.Debug.LogError("DeviceLocationProvider: " + "Timed out trying to initialize location services!");
            ...
            {
                UnityEngine.Debug.LogError("DeviceLocationProvider: " + "Failed to initialize location services!");
                _currentLocation.IsLocationServiceInitializing = false;
                _currentLocation.IsLocationServiceEnabled = false;
                SendLocation(_currentLocation);
                yield break;
